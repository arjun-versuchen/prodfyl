...
export default function SqlQuestionPage() {
  const { pathSlug = 'sql', id } = useParams()
  ...
  if (lockedSheet || lockedQuestion) {
    return (
      <>
        <SEO title={question.title} description="Premium SQL interview question" path={`/learn/${pathSlug}/question/${question.id}`} noIndex />
    ...
    <>
      ...
      {question.example && (
        <section className="mt-6">
          <h2 className="mb-3 text-sm font-semibold uppercase tracking-wide text-success">Example SQL</h2>
